...
<tr>
	<td>
		<table border="1" width="100%">
			...
			<?php
			$stmt = $dbh->prepare("SELECT *	FROM plane WHERE state='wait' ORDER BY turn");
		    $stmt->execute();
	...
	<td>
		<table border="1" width="100%">
			...
			<?php
			$stmt = $dbh->prepare("SELECT *	FROM plane WHERE state='play' ORDER BY turn");
		    $stmt->execute();
			while($db_field = $stmt->fetch()){
				?>
				<tr bgcolor=<?php if($odd){echo "#CCECCC";}else{echo "#F7F7F7";} $odd=!$odd;  ?> align="center" vAlign="top">
...
<tr>
	...
	<td>
		<table border="1" width="100%">
			...
			<?php
			$stmt = $dbh->prepare("SELECT *	FROM plane WHERE state='' ORDER BY turn");
		    $stmt->execute();
			while($db_field = $stmt->fetch()){
				?>
				<tr bgcolor=<?php if($odd){echo "#CCECCC";}else{echo "#F7F7F7";} $odd=!$odd;  ?> align="center" vAlign="top">
	...
	<td>
		<table border="1" width="100%">
			...
			<?php
			$stmt = $dbh->prepare("SELECT *	FROM plane WHERE state='done' ORDER BY turn");
		    $stmt->execute();
			while($db_field = $stmt->fetch()){
				?>
				<tr bgcolor=<?php if($odd){echo "#CCECCC";}else{echo "#F7F7F7";} $odd=!$odd;  ?> align="center" vAlign="top">
